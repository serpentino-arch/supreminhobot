...
class ItemsList:
    ...
    def render(self) -> None:
        ...
        # Handle case when no drops are available
        if self.date == "No drops available":
            with self.container:
                ui.label("No drops available for this season yet.").classes("text-gray-500 text-lg")
            return
...
with ui.element("div").classes("w-full p-8"):
    ...
    drop_dates = get_drop_dates() or ["No drops available"]
